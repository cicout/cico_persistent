...
class ORMDBServiceTests: XCTestCase {
    ...
    func test_Class_Array() {
        ...
        DebugAide.showDuration(closure: {
            let readObjects =
            self.service
                .readObjects(ofType: TCodableClass.self,
                             whereString: nil,
                             orderByName: "name",
                             descending: true,
                             limit: 10)
    ...
    func test_auto_increment() {
        var objects = [TAutoIncrement].init()
        for index in 0..<10 {
        ...
        let readObjects =
        self.service
            .readObjects(ofType: TAutoIncrement.self,
                             whereString: nil,
                             orderByName: "rowID",
                             descending: true,
                             limit: 10)
        XCTAssertNotNil(readObjects, "[FAILED]: read failed")
        XCTAssert(readObjects.count == 10, "[FAILED]: read failed")

        let object0 = readObjects[0]
        let object9 = readObjects[9]
...
extension ORMDBServiceTests {
    ...
    func test_Composite_Array() {
        ...
        DebugAide.showDuration(closure: {
            let readObjects =
            self.service
                .readObjects(ofType: TCompositeStruct.self,
                             whereString: nil,
                             orderByName: "name",
                             descending: true,
                             limit: 10)
